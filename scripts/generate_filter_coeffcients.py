...
def generate_source(filename_base, ext, template_dict):
    ...
    filename = os.path.join(source_dir, filename_base + ext)
    with open(filename, 'w') as f:
        f.write(template.render(template_dict))


def template_setup(ext):
    template_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                'templates')
    template_loader = jinja2.FileSystemLoader(template_dir)
    template_env = jinja2.Environment(loader=template_loader)
    return template_env.get_template('filterlowpass{}.in'.format(ext))
...
if __name__ == "__main__":
    ...
    for filter_type, filter_name in zip((butter, cheby1, cheby2),
        ...
        generate_source(filter_name.lower() + 'lowpass', '.cpp', template_dict)
        generate_source(filter_name.lower() + 'lowpass', '.h', template_dict)
